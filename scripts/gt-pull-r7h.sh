...
if [ ! -x "$PULL" ]; then
    ...
    echo "  sh scripts/build-r7h-known-resource-open.sh" >&2
    exit 1
fi
...
echo
echo "R7H Finder-style persistent recursive AFP copy"
...
echo "Data open:   R7G known-size AFP2 fast open"
echo "Rsrc open:   known-size AFP2 fast open + R4 stateful stream"
echo "Recovery:    R7D DID rebuild; fresh metadata where required"
