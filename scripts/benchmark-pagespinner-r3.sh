...
BUILD=${1:-"$ROOT/build-rfork-r3"}
BIN="$BUILD/gt-afp-pull"
OUTROOT=${OUTROOT:-/mnt/AFPSERVER/128G2/gt-afp-r3-test}
...
if [ ! -x "$BIN" ]; then
    ...
    echo "Run first: sh scripts/build-rfork-r3.sh" >&2
    exit 1
fi
...
DEST="$OUTROOT/PageSpinner-r3-$STAMP"
LOG="$OUTROOT/pagespinner-r3-$STAMP.log"
mkdir -p "$DEST"
