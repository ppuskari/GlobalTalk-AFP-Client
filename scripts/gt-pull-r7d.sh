...
if [ ! -x "$PULL" ]; then
    ...
    echo "  sh scripts/build-r7d-finder-recovery-did.sh" >&2
    exit 1
fi
...
echo
echo "R7D Finder-style persistent recursive AFP copy"
echo "Remote base: $SOURCE"
echo "Local base:  $DEST"
echo "Session:     one login/attach for the recursive walk"
echo "Traversal:   R7C FPEnumerate metadata + ParentDirID/DID reuse"
echo "Recovery:    rebuild parent DID chain after reconnect/reattach"
