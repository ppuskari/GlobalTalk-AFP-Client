...
if [ ! -x "$PULL" ]; then
    ...
    echo "  sh scripts/build-r7c-finder-did.sh" >&2
    exit 1
fi
...
echo
echo "R7C Finder-style persistent recursive AFP copy"
echo "Remote base: $SOURCE"
echo "Local base:  $DEST"
echo "Session:     one login/attach for the recursive walk"
echo "Traversal:   reuse FPEnumerate metadata and ParentDirID/DID cache"
echo "Path work:   no redundant per-file stat; no repeated parent full-path walk"
