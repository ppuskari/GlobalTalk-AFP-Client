...
if [ ! -x "$PULL" ]; then
    echo "ERROR: downloader not found; build R7FB first" >&2
    exit 1
fi
...
echo
echo "R7FB diagnostic: R7FA + resource-length FPEnumerate field"
echo "Remote base: $SOURCE"
echo "Local base:  $DEST"
echo "Xattrs:      generic stage skipped"
echo "Enumeration: data length + resource length; no FinderInfo"
echo "Reuse:       resource length carried only, NOT consumed"
