...
#
# R2F clean-build DDP URL restoration.
#
# lib/afp_url.c is refreshed from the pinned Netatalk Client checkout before
# R2F is built so no diagnostic-era edits can leak into the correctness target.
# The upstream file does not know about afp+ddp:// URLs; that parser is part of
# GlobalTalk-AFP-Client's overlay. Recreate it here with the Jessie-proven
# rooted pathname semantics already included.
#
...
MARKER = "GLOBALTALK RFORK R2F ROOTED DDP URL"


def die(msg):
    raise SystemExit("apply_rfork_r2f_ddp_url: " + msg)
...
# If the DDP helper already exists, normalize only its pathname behavior.
if "static int parse_ddp_url(" in text:
    if rooted in text:
        print("R2F rooted DDP URL parser already present: {}".format(PATH))
        raise SystemExit(0)
    ...
    print("Updated existing DDP URL parser to rooted paths: {}".format(PATH))
    raise SystemExit(0)

# Clean pinned upstream source: recreate the GlobalTalk DDP URL overlay.
...
helper = r'''/* GLOBALTALK RFORK R2F ROOTED DDP URL */
